  ...
  return (
    <section className="animate-fade-up animate-fade-up-delay-3 relative mt-7 rounded-[24px] p-px shadow-[0_20px_50px_rgba(118,146,255,0.18)]"
      ...
      <div className="relative grid grid-cols-[auto_1fr_auto] items-center gap-7 overflow-hidden rounded-[23px] p-10 max-sm:grid-cols-1 max-sm:gap-4 max-sm:p-7"
        ...
        <div className="relative z-[1] flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-[18px] text-paper shadow-[0_12px_30px_rgba(27,44,193,0.45),inset_0_1px_0_rgba(255,255,255,0.16)]"
        ...
        {alertStatus === "done" ? (
          ...
          <form onSubmit={onSubmit} className="relative z-[1] flex items-stretch gap-2 max-sm:flex-col">
            <input
              ...
              required
              className="h-[50px] w-[240px] rounded-full border border-paper/[0.14] bg-paper/[0.06] px-[18px] text-[14.5px] text-paper outline-none transition-all duration-[0.18s] placeholder:text-paper/40 focus:border-sky focus:bg-paper/[0.10] focus:shadow-[0_0_0_3px_rgba(171,210,250,0.18)] max-sm:w-full"
            />
            <button
              type="submit"
              disabled={alertStatus === "submitting"}
              className="inline-flex h-[50px] items-center gap-1.5 rounded-full bg-paper px-[22px] text-[14px] font-semibold text-ink-900 transition-all duration-[0.18s] hover:bg-surface hover:shadow-[0_8px_20px_rgba(255,255,255,0.15)] disabled:opacity-50"
            >
