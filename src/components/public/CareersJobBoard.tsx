...
export function CareersJobBoard({ jobs }: Props) {
  ...
  return (
    <div className="mt-6 space-y-8">
      <section
        className="flex flex-col gap-3 rounded-2xl bg-white px-4 py-4 sm:flex-row sm:items-center sm:px-5 sm:py-3"
      ...
      {totalCount === 0 ? (
        <div className="rounded-2xl border border-slate-700/30 bg-slate-800/30 px-8 py-12 text-center">
          <p className="text-sm text-slate-400">
            ...
            <button
              ...
              className="ml-1 text-[#3b82f6] underline hover:text-[#60a5fa]"
            >
              Clear filters
            </button>
          </p>
        ...
        <div>
          <div className="mb-4">
            ...
            <p className="text-sm text-slate-400">{totalCount} {totalCount === 1 ? "job" : "jobs"} available</p>
          </div>
          ...
          <div className="space-y-8">
            {[...grouped.entries()].map(([department, deptJobs]) => (
              <section key={department}>
                <h3
                  className="mb-3 text-[0.75rem] font-semibold uppercase tracking-[0.08em] text-slate-400"
                >
                  {department}
                </h3>
                <div className="divide-y divide-slate-800">
                  {deptJobs.map((job) => (
                    <Link
                      ...
                      className="flex flex-col gap-1 py-3.5 transition-colors hover:bg-white/5 px-3 -mx-3 rounded-lg group"
                    >
                      <div className="flex items-baseline gap-2.5">
                        <span className="text-base font-semibold text-[#3b82f6] group-hover:text-[#60a5fa]">
                          {job.title}
                        </span>
                        <span className="text-sm text-slate-500">/{job.slug}</span>
                      </div>
                      {job.location && (
      ...
      <div
        className="flex flex-col gap-4 rounded-[20px] p-5 sm:flex-row sm:items-center"
      ...
      >
        ...
        {alertStatus === "done" ? (
          <p className="text-sm font-medium text-emerald-600 whitespace-nowrap">Subscribed! Check your email.</p>
        ) : (
          <form onSubmit={handleAlertSubmit} className="flex flex-col gap-2 sm:flex-row sm:items-center">
            ...
            <button
              ...
              className="h-12 rounded-full bg-[#2563eb] px-6 text-sm font-medium text-white hover:bg-[#1d4ed8] transition-colors disabled:opacity-50 whitespace-nowrap"
            >
        ...
        {alertStatus === "error" && (
          <p className="text-sm text-red-600 whitespace-nowrap">Something went wrong. Please try again.</p>
        )}
      </div>
    </div>
  );
