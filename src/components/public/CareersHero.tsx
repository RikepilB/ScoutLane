...
export function CareersHero({ count, session }: CareersHeroProps) {
  return (
    <header className="border-b border-border-dark bg-surface-inverse px-6 pb-11 pt-10 sm:px-10">
      <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex items-center gap-2.5">
          <Link
            ...
            className="inline-flex h-11 items-center rounded-control bg-primary px-5 text-sm font-medium text-primary-foreground no-underline transition-colors hover:bg-brand-royal-hover"
          >
            {session?.user ? "Dashboard" : "Sign in"}
          </Link>
          <Link
            href={session?.user ? "/admin/jobs/new" : "/signin"}
            className="inline-flex h-11 items-center gap-1.5 rounded-control border border-border-dark-strong px-5 text-sm font-medium text-paper/80 no-underline transition-colors hover:border-sky/40 hover:text-paper"
          >
