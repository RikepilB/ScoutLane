...
const buttonVariants = cva(
  ...
  {
    variants: {
      // Flat by direction — no drop shadow on a control. Weight alone
      // separates the primary action, so a view can have exactly one.
      variant: {
        default: "bg-brand-royal text-paper hover:bg-brand-royal-hover",
        destructive: "bg-danger text-paper hover:bg-danger/90",
        outline: "border border-mist bg-surface text-ink-900 hover:border-mist-strong hover:bg-paper",
        secondary: "bg-paper-2 text-ink-900 hover:bg-mist",
        ghost: "text-ink-700 hover:bg-paper-2 hover:text-ink-900",
