      ...
      {rules.map((rule) => (
        <div key={rule.id} className="flex items-center justify-between gap-3 rounded-2xl border border-border/70 bg-card p-4 shadow-sm">
          <div className="flex items-center gap-2">
            <span className={`inline-flex h-2 w-2 rounded-full ${rule.active ? "bg-success" : "bg-mist"}`} />
          ...
          <button
            ...
            disabled={isPending}
            className="rounded-lg p-2 text-danger hover:bg-danger-soft disabled:opacity-50"
            title="Delete rule"
          >
