...
const accentMap: Record<StatCardProps["accent"], { bg: string; fg: string }> = {
  green: { bg: "bg-[rgba(5,150,105,0.12)]", fg: "text-[#059669]" },
  amber: { bg: "bg-[rgba(217,119,6,0.12)]", fg: "text-[#D97706]" },
  cyan: { bg: "bg-[rgba(2,132,199,0.12)]", fg: "text-[#0284C7]" },
  peri: { bg: "bg-[rgba(163,176,255,0.14)]", fg: "text-[#1A2EFF]" },
};

function StatCard({ label, value, hint, icon: Icon, accent }: StatCardProps) {
  const a = accentMap[accent];
  return (
    <article className="flex items-start gap-3.5 rounded-2xl border border-[#E5E7EB] bg-white p-[18px] shadow-[0_1px_3px_rgba(9,21,64,0.06),0_1px_2px_rgba(9,21,64,0.04)] transition-all hover:-translate-y-0.5 hover:shadow-[0_4px_12px_rgba(9,21,64,0.08),0_2px_4px_rgba(9,21,64,0.04)]">
      ...
      <div>
        ...
        <div className="mt-1 text-[30px] leading-none tracking-[-0.02em] text-[#0B1437]" style={{ fontFamily: "var(--font-display)", fontWeight: 500 }}>
          {value}
        </div>
        {hint ? <div className="mt-1.5 text-[11.5px] text-[#6B7280]">{hint}</div> : null}
      </div>
    </article>
  );
}
    ...
    <main className="flex-1" style={{ background: "#F9FAFB" }}>
      <div className="mx-auto flex w-full max-w-[1100px] flex-col gap-6 px-10 py-8">
        ...
        <section className="animate-fade-up relative overflow-hidden rounded-[24px] p-9 text-white shadow-[0_12px_32px_rgba(9,21,64,0.10),0_4px_8px_rgba(9,21,64,0.06)]"
          style={{
            background: "radial-gradient(circle at 8% 20%, rgba(43,75,255,0.45), transparent 55%), linear-gradient(170deg, #0F1B4D 0%, #0B1437 70%)",
          }}>
          <div className="pointer-events-none absolute inset-0"
            style={{
              backgroundImage: "linear-gradient(rgba(163,176,255,0.05) 1px, transparent 1px), linear-gradient(90deg, rgba(163,176,255,0.05) 1px, transparent 1px)",
          ...
          <div className="relative flex flex-wrap items-end justify-between gap-6">
            <div>
              <div className="mb-3.5 flex items-center gap-2.5 text-[11px] font-medium uppercase tracking-[0.16em] text-[#A3B0FF]"
                style={{ fontFamily: "var(--font-mono)" }}>
                <span className="h-px w-5 bg-[#A3B0FF]" />
                Recruitment cockpit
              </div>
        ...
        {/* Manage card */}
        <section className="animate-fade-up animate-fade-up-delay-3 flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-dashed border-[#E5E7EB] bg-white px-6 py-5 shadow-[0_1px_3px_rgba(9,21,64,0.06),0_1px_2px_rgba(9,21,64,0.04)]">
          <div>
            <h2 className="text-[15px] font-semibold text-[#0B1437]" style={{ fontFamily: "var(--font-display)" }}>
              Manage all jobs
            </h2>
          ...
          <Button asChild variant="outline" className="gap-1.5 rounded-lg border-[#E5E7EB] text-[#0B1437] hover:bg-[#F9FAFB]">
