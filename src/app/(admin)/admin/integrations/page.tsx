            ...
            <table className="w-full text-[13.5px]">
              ...
              <tbody>
                {integrations.map((integration) => {
                  ...
                  return (
                    <tr key={integration.id} className="border-b border-[rgba(9,21,64,0.06)] transition-colors hover:bg-paper">
                      <td className="px-4 py-3.5">
                        <a
                          href={`/admin/jobs/${integration.job.id}/integrations`}
                          className="text-[13.5px] font-medium text-ink-900 hover:text-brand-royal"
                        >
                      ...
                      <td className="px-4 py-3.5 max-w-xs truncate text-[12px] text-steel"
                        ...
                        {lastLog ? (
                          <span
                            className={`inline-flex items-center gap-1 text-xs ${
                              lastLog.status < 400 ? "text-success" : "text-danger"
                            }`}
                          >
                            <span
                              className={`inline-flex h-1.5 w-1.5 rounded-full ${
                                lastLog.status < 400 ? "bg-success" : "bg-danger"
                              }`}
                            />
                            {lastLog.status} —{" "}
